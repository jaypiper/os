...
void sdcard_read_sector(uint8 *buf, int sectorno) {
	...
	if (0 == timeout) {
		Assert(0, "sdcard: timeout waiting for reading");
	}
...
void sdcard_write_sector(uint8 *buf, int sectorno) {
	...
	if (0 == timeout) {
		// releasesleep(&sdcard_lock);
		Assert(0, "sdcard: invalid response token");
	}
	...
	if (0 == timeout) {
		// releasesleep(&sdcard_lock);
		Assert(0, "sdcard: timeout waiting for response");
	}
	sd_end_cmd();
	...
	if (0 != result || 0 != error_code) {
		...
		printf("result: %x\n", result);
		printf("error_code: %x\n", error_code);
		Assert(0, "sdcard: an error occurs when writing");
	}
