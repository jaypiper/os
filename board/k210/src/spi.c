...
#include <string.h>
#include <kernel.h>
#define GPIOHS_V                (0x38001000 )
#define DMAC_V                  (0x50000000 )
#define GPIO_V                  (0x50200000 )
#define SPI_SLAVE_V             (0x50240000 )
#define FPIOA_V                 (0x502B0000 )
#define SPI0_V                  (0x52000000 )
#define SPI1_V                  (0x53000000 )
#define SPI2_V                  (0x54000000 )
#define SYSCTL_V                (0x50440000 )
                            ...
                            uint64 cmd_len, const uint8 *tx_buff, uint64 tx_len)
{
    ...
    uint8 *v_buf = pmm->alloc(cmd_len + tx_len);
    uint64 i;
    for(i = 0; i < cmd_len; i++)
        v_buf[i] = cmd_buff[i];
    for(i = 0; i < tx_len; i++)
                              ...
                              const void *tx_buff, uint64 tx_len, spi_transfer_width_t spi_transfer_width)
{
    ...
    switch(spi_transfer_width)
    {
        case SPI_TRANS_SHORT:
            ...
            buf = pmm->alloc((tx_len) * sizeof(uint32_t));
            for(i = 0; i < tx_len; i++)
                                   ...
                                   uint64 cmd_len, uint8 *rx_buff, uint64 rx_len)
{
    ...
    {
        case SPI_TRANS_INT:
            write_cmd = pmm->alloc(cmd_len + rx_len);
            ...
            break;
        case SPI_TRANS_SHORT:
            write_cmd = pmm->alloc((cmd_len + rx_len) / 2 * sizeof(uint32_t));
            ...
            break;
        default:
            write_cmd = pmm->alloc((cmd_len + rx_len) * sizeof(uint32_t));
            for(i = 0; i < cmd_len; i++)
                                ...
                                const uint8 *cmd_buff, uint64 cmd_len, const uint8 *tx_buff, uint64 tx_len)
{
    ...
    switch(frame_width)
    {
        case SPI_TRANS_INT:
            ...
            break;
        case SPI_TRANS_SHORT:
            buf = pmm->alloc((cmd_len + tx_len) / 2 * sizeof(uint32_t));
            for(i = 0; i < cmd_len / 2; i++)
            ...
            break;
        default:
            buf = pmm->alloc((cmd_len + tx_len) * sizeof(uint32_t));
            for(i = 0; i < cmd_len; i++)
                buf[i] = cmd_buff[i];
            for(i = 0; i < tx_len; i++)
